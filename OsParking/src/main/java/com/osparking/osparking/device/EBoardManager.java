...
import static com.osparking.global.names.OSP_enums.EBD_Row.*;
...
public class EBoardManager extends Thread implements
    ...
    private static byte[] getEBDSimulatorDefaultMessage(byte deviceNo, EBD_Row row, int msgSN) {
        ...
        logParkingException(Level.SEVERE, null, "E wholeMessageLen: " + wholeMessageLen);        
            ...
            try {
                ...
                synchronized(mainForm.getSocketMutex()[E_Board.ordinal()][deviceNo]) 
                {
                    ...
                    {      
                        ...
                        case EBD_ACK: // e-board acknowledges an e-board display message reception.
    ...
    public void writeMessage(MsgCode code, byte[] msgBytes) {
        try {
            if (DEBUG) {
                System.out.println("From OsParking to EBD #" + deviceNo + ": " + code);
            }
            ...
            if (mainForm != null) {
                mainForm.getSendEBDmsgTimer()[deviceNo][row.ordinal()].reschedule(
                            new SendEBDMessageTask(
                                mainForm, deviceNo, (row == TOP ? EBD_DEFAULT1 : EBD_DEFAULT2),
                                getEBDSimulatorDefaultMessage(deviceNo, row, 
    ...
    public void showDefaultMessage() { 
        sendEBoardDefaultSetting(mainForm, deviceNo, TOP);
        sendEBoardDefaultSetting(mainForm, deviceNo, BOTTOM);
    }
